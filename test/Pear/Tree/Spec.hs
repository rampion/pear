...
spec = describe "Pear.Tree" do
  ...
  describe "??" do
    Foldable.for_ 
      ...
      , ( Top (('a' :× 'b') :× ('c' :× 'd')) :⊢ Just ('e' :× 'f') :⊢ Just 'g'
        , "abcdefg"
        )
      , ( Top ((('a' :× 'b') :× ('c' :× 'd')) :× (('e' :× 'f') :× ('g' :× 'h'))) :⊢ Nothing :⊢ Nothing :⊢ Just 'i'
        , "abcdefghi"
        )
      ] \(ta, as) -> do

      let n = toEnum (length as)

      describe ("for a " <> show n <> "-element tree") do

        it ("finds the elements at indexes 0 through " <> show (n - 1)) do
          [ta ?? i | i <- [0 .. n - 1]] `shouldBe` Just <$> as

        it ("finds nothing at the indexes " <> show n <> " through " <> show (2*n -1)) do
          [ta ?? i | i <- [n .. 2*n - 1]] `shouldBe` Nothing <$ as

  describe "fuse" do
    it "combines two one-element trees as (<>) does" do
      fuse (Top 'a') (Top 'b') `shouldBe` Top ('a' :× 'b') :⊢ Nothing

    it "combines a two-element tree and a one-element tree as (<>) does" do
      fuse (Top ('a' :× 'b') :⊢ Nothing) (Top 'c') `shouldBe` Top ('a' :× 'b') :⊢ Just 'c'

    it "combines a one-element tree and a two-element tree differently than (<>)" do
      fuse (Top 'c') (Top ('a' :× 'b') :⊢ Nothing) `shouldBe` Top ('a' :× 'b') :⊢ Just 'c'

    it "combines a three-element tree and a two-element tree differently than (<>)" do
      fuse (Top ('a' :× 'b') :⊢ Just 'e') (Top ('c' :× 'd') :⊢ Nothing) 
        `shouldBe` Top (('a' :× 'b') :× ('c' :× 'd')) :⊢ Nothing :⊢ Just 'e'

    it "combines a one-element tree and a three-element tree differently than (<>)" do
      fuse (Top 'c') (Top ('a' :× 'b') :⊢ Just 'd') 
        `shouldBe` Top (('a' :× 'b') :× ('c' :× 'd')) :⊢ Nothing :⊢ Nothing

  describe "fiss" do
    it "can split a two-element tree into two one-element trees" do
      fiss (literal @1) (Top ('a' :× 'b') :⊢ Nothing) `shouldBe`
        Just (Top 'a', Top 'b')

    it "can split a three-element tree into a two-element tree and a one-element tree" do
      fiss (literal @2) (Top ('a' :× 'b') :⊢ Just 'c') `shouldBe`
        Just (Top ('a' :× 'b') :⊢ Nothing, Top 'c')

    it "can split a three-element tree into a one-element tree and a two-element tree" do
      fiss (literal @1) (Top ('a' :× 'b') :⊢ Just 'c') `shouldBe`
        Just (Top 'c', Top ('a' :× 'b') :⊢ Nothing)

    it "can split a five-element tree into a three-element tree and a two-element tree" do
      fiss (literal @3)  (Top (('a' :× 'b') :× ('c' :× 'd')) :⊢ Nothing :⊢ Just 'e')
        `shouldBe` Just (Top ('a' :× 'b') :⊢ Just 'e', Top ('c' :× 'd') :⊢ Nothing) 

  describe "fromNonEmpty" do
    it "can create a seven-element tree" do
      fromNonEmpty ('a' :| "bcdefg") `shouldBe`
        Top (('a' :× 'b') :× ('c' :× 'd')) :⊢ Just ('e' :× 'f') :⊢ Just 'g'

  describe "generate" do
    it "can create a seven-element tree" do
      generate (literal @7) id `shouldBe`
        Top ((0 :× 1) :× (2 :× 3)) :⊢ Just (4 :× 5) :⊢ Just 6

    it "can create a ten-element tree" do
      generate (literal @10) id `shouldBe`
        Top (((0 :× 1) :× (2 :× 3)) :× ((4 :× 5) :× (6 :× 7))) :⊢ Nothing :⊢ Just (8 :× 9) :⊢ Nothing
